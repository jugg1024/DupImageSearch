...
for cato_count = 3 : length(cato_names)
    ...
    for dirc = 3:length(dirnames)
        ...
        matName = strcat(dataset,'/',feature_name,'/',cato,'_',filename,'_feat_CM.mat');
        if  exist(matName) == 0
...
for i = 1:length(matnames)
    if strcmp(matnames{i, 1},'feat_CM.mat')
        ;
    else
        load(strcat(dataset,'/',feature_name,'/',matnames{i, 1}));
...
matName = strcat(dataset,'/',feature_name,'/','feat_CM.mat');
